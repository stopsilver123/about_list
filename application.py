...
def list_restaurants():
    ...
    avg_rate=DB.get_avgrate_byname(data)
    page_count = len(data)
    print(tot_count,page_count)
    return render_template("EDlist.html", datas=data.items(), avg_rate=avg_rate,
                           total=tot_count, limit=limit, page=page, page_count=math.ceil(tot_count/6), category=category)
...
@application.route("/EDdetail_notLogin/<name>/") #리스트에서 넘어옴
...
def ED_register_post():
    ...
    if DB.insert_restaurant(data['restaurant_name'], data, image_file.filename):
        return render_template("EDdetail.html", data=data, image_path="static/images/"+image_file.filename)
    else:
...
def view_reviews(res_name):
    ...
    return render_template("EDMOREreview.html", data2_restaurant_name=data_Rname2, datas=data, total=tot_count)
...
#계산기 화면
@application.route("/EDrecommend", methods=['POST'])
def recommend_cost_post():
    data=request.form
    return render_template("EDrecommendResult.html", data=data)
